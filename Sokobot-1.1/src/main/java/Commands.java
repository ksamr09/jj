...
public class Commands extends ListenerAdapter {
    ...
    ArrayList<String> commandsPrefix = new ArrayList<String>(Arrays.asList("play", "con", "stop"));
        ...
        if (args[0].toLowerCase().equals(Bot.getPrefix(event.getGuild()) + "prefix"))
        {
            if (event.getMember().hasPermission(Permission.ADMINISTRATOR)) {
                if (args.length == 2 && args[1].length() == 1) {
                    ...
                    event.getChannel().sendMessage("تم تغير البريفكس ``" + Bot.getPrefix(event.getGuild()) + "``.").queue();
                } else {
                    event.getChannel().sendMessage("``" + args[1] + "`` البريفكس غير صالح!").queue();
                }
                event.getMessage().delete().queue();
            }
            else
            {
                event.getChannel().sendMessage(event.getAuthor().getAsMention() + ", ليس لديك الصلاحية لاستخدام هذا الامر.").queue();
            }
        }
    ...
    EmbedBuilder info(Guild guild)
    {
        ...
        info.setDescription("Eric's game bot .");
        ...
        info.addField("Add to your server", "send ,sg to Eric#4750", false);
        info.addField("Source code", "", false);
        info.setFooter("change by Eric", "");
        return info;
    }
    ...
    public static void sendWinEmbed(Guild guild, Message message, String level)
    {
        ...
        embed.setDescription("Type ``" + Bot.getPrefix(guild) + "con`` to continue to Level " + level + " or ``" + Bot.getPrefix(guild) + "stop`` to quit ");
